...
def generate_binary_latent_codes(code_length):
    num_codes = 2 ** code_length
    indices = torch.arange(num_codes).unsqueeze(1)  # shape: (num_codes, 1)
    powers = 2 ** torch.arange(code_length - 1, -1, -1)  # shape: (code_length,)
    binary_codes = (indices & powers).ne(0).float()  # shape: (num_codes, code_length)
    return binary_codes 